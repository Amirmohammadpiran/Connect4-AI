...
def minimax_with_alpha_beta(map, isAI_Turn, depth, alpha, beta):
    ...
    if(isAI_Turn):
        ...
        for n in nodes:
            value = min(value, minimax_with_alpha_beta(n, False, depth-1, alpha, beta))
            beta = min(beta, value)
            if (alpha >= beta):
                break
        return value
        ...
        for n in nodes:
            value = max(value, minimax_with_alpha_beta(n, True, depth-1, alpha, beta))
    ...
    while(not filled_map(map)):
        ...
        if(player1turn):
            ...
            for s in successor(map, 2):
                score = minimax_with_alpha_beta(s, False, 3, float('inf'), float('-inf'))
                if(score < best_score):
                    best_choice = s
                    best_score = score
